...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        retriever, not the answer the user actually receives.
        """
...
def _coverage(reference: set[str], covered_by: set[str]) -> float:
    """Fraction of ``reference`` tokens that also appear in ``covered_by``.

    Every answer-side metric in this lab is the same shape — "how much of X is
    supported by Y" — so they all delegate here. An empty reference means there
    is nothing to check, which scores 1.0 by convention.
    """
    if not reference:
        return 1.0
    ratio = len(reference & covered_by) / len(reference)
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # Steps 2–3 — Average Precision: only positions holding a relevant
        # chunk contribute, so relevant chunks placed early score higher.
        hits = 0
        precision_sum = 0.0
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # sorted() is stable, so chunks tied on overlap keep their retriever order.
    return sorted(
        contexts,
...
class LLMJudge:
    ...
    # A judge that only ever returns 5s is useless, so the prompt below states
    # the scale explicitly and demands a rationale before the scores.
    PROMPT_TEMPLATE = """You are a strict evaluator of a Student Services AI assistant.
...
Scoring scale:
  5 = correct, complete, grounded in policy, no unsupported claim
  4 = mostly correct, minor gap that does not mislead the student
  3 = partially correct, a relevant condition or exception is missing
  2 = significant error or most of the expected content missing
  1 = wrong, off-topic, or fabricated

Judge only substance. Length, formatting and confident tone must NOT raise the
score. Reply with JSON only: {{"<criterion>": <score>, ...}}"""
    ...
    def score_response(
        ...
        prompt = self.PROMPT_TEMPLATE.format(
            question=question,
            answer=answer,
            rubric=rubric_text or "- overall: Is the answer correct and useful?",
        )
        raw = self.judge_llm_fn(prompt)
        reasoning = raw if isinstance(raw, str) else str(raw)

        parsed = self._parse_scores(reasoning)
        criteria = list(rubric) or list(parsed) or ["overall"]
        # Unparseable judge output must not crash the benchmark: fall back to
        # the neutral 0.5 so the case shows up as "needs human review".
        scores = {
            criterion: parsed.get(criterion, 0.5) for criterion in criteria
        }
        return {"scores": scores, "reasoning": reasoning}

    @staticmethod
    def _parse_scores(raw: str) -> dict[str, float]:
        """Extract a criterion → score-in-[0,1] mapping from the judge reply."""
        ...
        for criterion, value in payload.items():
            ...
            if score > 1.0:  # judge answered on the 1-5 rubric scale
                score = (score - 1.0) / 4.0
            scores[str(criterion)] = max(0.0, min(1.0, score))
        return scores
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def mean_score(entry: dict[str, Any]) -> float:
            values = [
                float(value)
                for value in (entry.get("scores") or {}).values()
                if isinstance(value, (int, float)) and not isinstance(value, bool)
            ]
            return sum(values) / len(values) if values else 0.0

        means = [mean_score(entry) for entry in scores_batch or []]
        if not means:
            return {
        ...
        # Position bias: the response judged first sits noticeably above the rest.
        rest = means[1:]
        positional = bool(rest) and (means[0] - sum(rest) / len(rest)) > 0.1
        return {
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the ORIGINAL pair: it carries metadata (id, difficulty)
            # that the report and the failure analysis need.
            result.qa_pair = pair
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        A drop of exactly 0.05 is tolerated; only a LARGER drop regresses.
        """
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        """
        counts = Counter(
            failure.failure_type or "unclassified" for failure in failures or []
        )
        return dict(counts)
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Two or more broken metrics rarely share a single fix — say so instead
        # of blaming whichever one happens to be lowest.
        if sum(1 for score in scores.values() if score < 0.5) >= 2:
            return "Multiple issues detected — review full pipeline"

        weakest = min(scores, key=lambda metric: scores[metric])
        return {
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures or []):
            suggestion = (
                ...
                else "See prioritized improvement suggestions"
            )
            lines.append(
                f"| F{index + 1:03d} "
    ...
    def generate_improvement_suggestions(
        ...
        # Most frequent failure type first: fixing one root cause usually
        # clears a whole cluster of cases.
        playbook = {
            "hallucination": (
                "Add a grounding guardrail that drops sentences whose claims are "
                "not supported by the retrieved chunks, and force policy citations"
            ),
            "irrelevant": (
                "Rewrite the system prompt to restate the student's question and "
                "answer it directly before adding any policy background"
            ),
            "incomplete": (
                "Raise top_k and chunk overlap so multi-document procedures "
                "(deadline + fee + form) arrive together, and ask the generator "
                "for every condition and exception"
            ),
            "off_topic": (
                "Add intent routing so registration, tuition and grading "
                "questions hit the right document set"
            ),
            "refusal": (
                "Relax the scope guardrail so in-scope questions are answered "
                "instead of deflected to the help desk"
            ),
            "unclassified": (
                "Inspect the answer/context trace for these cases and extend the "
                "failure taxonomy so they can be triaged automatically"
            ),
        }
        suggestions = [
        ...
        # Always leave the student with at least three actionable items.
        fallbacks = [
            "Add few-shot examples of complete, well-cited Student Services answers",
            "Log Context Recall per case and alert when the retriever misses "
            "gold evidence",
            "Grow the golden dataset with these failures so regressions are caught "
            "by the CI quality gate",
        ]
